...
export class Game {
    ...
    private static isSpaceStar(space: number) {
    ...
    public getPossibleMoves(roll): Move[] {
        const moves: Move[] = [];
        ...
        for (let pieceId = 0; pieceId < team.pieces.length; pieceId++) {
            ...
            if (!team.isPieceOnSpace(newSpace)) {
                const enemy = otherTeam.getPieceOnSpace(newSpace);
...
export class Team {
    ...
    public addPiece(piece: number) {
        this.pieces.push(piece);
    }

    public killPiece(n: number) {
        this.pieces.splice(n, 1);
    }

    public isPieceOnSpace(space: number) {
        return this.pieces.indexOf(space) >= 0;
    }

    public getPieceOnSpace(space: number) {
